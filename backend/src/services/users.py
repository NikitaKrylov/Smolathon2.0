...
async def auth_user(login: str, password: str) -> UserOut:
    async with async_session() as session:
        user = await users.get_by_login(session, login)
        if not user:
            raise HTTPException(
...
async def register_user(user: UserCreate) -> UserOut:
    ...
    async with async_session() as session:
        existed_user = await base_db.get_by(session, User, (User.login == user.login), UserOut)
        if existed_user:
